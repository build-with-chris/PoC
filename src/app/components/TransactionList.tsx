...
type SerializedTransaction = Omit<transactions, 'amount' | 'date' | 'createdAt' | 'updatedAt' | 'settledAt'> & {
  amount: number
  date: string
  createdAt: string
  updatedAt: string
...
type TransactionWithCategory = transactions & { categories: categories }
              ...
              {transactions.map((t) => (
                <tr key={t.id} className="border-b border-zinc-100 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
                  ...
                  <td className="py-3 px-4 text-sm">
                    <span className={`inline-block px-2 py-1 rounded text-xs font-medium ${t.categories.type === 'income' ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-300'}`}>
                      {t.categories.name}
                    </span>
                  </td>
                  <td className={`py-3 px-4 text-sm font-medium ${t.categories.type === 'income' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
                    {t.categories.type === 'income' ? '+' : '-'}{Number(t.amount).toFixed(2)} €
                  </td>
